...
export async function teardown(app: ElectronApplication, testInfo: TestInfo) {
  ...
  if (currentTest?.file === file) {
    ...
    console.log(`Test ${ path.basename(file) } took ${ string }.`);
  } else {
    console.log(`Test ${ path.basename(file) } did not have a start time.`);
  }
}
...
export async function startRancherDesktop(testInfo: TestInfo, options: startRancherDesktopOptions = {}): Promise<ElectronApplication> {
  ...
  await fs.promises.rm(logsDir, { recursive: true, force: true, maxRetries: 3 });
